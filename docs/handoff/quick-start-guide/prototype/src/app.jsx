...
function App() {
  ...
  const [savingsGoalModal, setSavingsGoalModal] = React.useState(false);
  const [savingsGoalConfig, setSavingsGoalConfig] = React.useState(
    window.HZ.savingsGoalConfig
  );
  ...
  const ui = {
    ...
    mortgage,
    editSavingsGoal: () => setSavingsGoalModal(true),
    savingsGoal: window.HZ.computeSavingsGoal(savingsGoalConfig),
    manageCategories: () => setCategoryModal(true),
  ...
  else if (route === "history") screen = <History go={go} ui={ui} />;
  ...
  return (
    <div style={{ display: "flex", height: "100%", background: A.color.ink0 }}>
      <Sidebar route={route} go={go} onOpenManual={() => setManualOpen(true)} />
      <main
        ref={scrollRef}
      ...
      {savingsGoalModal && (
        <window.SavingsGoalModal
          onClose={() => setSavingsGoalModal(false)}
          config={savingsGoalConfig}
          onSave={(cfg) => {
            setSavingsGoalConfig(cfg);
            setSavingsGoalModal(false);
            notify("Savings goal updated", { variant: "success" });
          }}
        />
      )}
      {importWizard && (

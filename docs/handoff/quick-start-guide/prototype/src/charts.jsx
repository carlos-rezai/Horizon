...
/* ---------- History chart (past-only, no payoff/freedom decoration) ----------
   series: [{ key, name, color, kind:'liquid'|'account'|'debt', width, dashed }]
   visible: { [key]: bool }   onToggle(key)   onSolo(key) */
function HistoryChart({
  ...
  // Jan ticks (one per year in range)
  const years = [];
  data.forEach((d, i) => {
    if (d.month === 0 || i === 0) years.push({ i, year: d.year });
  });
  const tickStep = Math.max(1, Math.ceil(years.length / 8));
  const shownYears = years.filter((_, k) => k % tickStep === 0);

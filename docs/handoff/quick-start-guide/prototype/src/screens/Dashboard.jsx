...
function MortgageCountdown({ ui }) {
  ...
  const startedMonths = (2026 - sd.getFullYear()) * 12 + (6 - sd.getMonth());
...
function SavingsGoalCard({ ui }) {
  const { Card, Icon, ProgressBar, Money, Avatar, Badge } = window;
  const HZ = window.HZ;
  const goal = ui.savingsGoal;
  const [open, setOpen] = React.useState(false);
  const [eh, setEh] = React.useState(false);
  const s = goal.streak;
  const isRecord = s.current > 0 && s.current >= s.best;
      ...
      >
        ...
        >
          ...
          <div>
            ...
            >
              <span style={{ ...D.type.h1, color: D.color.text }}>
                Savings Streak
              </span>
              <span
                className="mono"
                style={{ fontSize: 15, color: D.color.accent, fontWeight: 600 }}
              >
                {s.current} mo
              </span>
              <span
              ...
              >
                {isRecord ? "matching your best" : `best ${s.best} mo`}
              </span>
            </div>
          </div>
        </button>
      ...
      <div style={{ display: "flex", gap: 5, marginTop: 18 }}>
        {s.yearTicks.map((m, i) => {
          ...
          return (
            <div
              key={i}
              title={`${MONTHS_FULL_DASH[m.month]} ${m.year} · ${upcoming ? "upcoming" : met ? "goal met" : "goal missed"}`}
              style={{
                flex: 1,
                height: 30,
                ...
                background: met
                  ...
                  : upcoming
                    ? "transparent"
                    : D.color.ink3,
                border: upcoming ? `1px dashed ${D.color.lineFaint}` : "none",
                opacity: met ? 0.45 + (i / s.yearTicks.length) * 0.55 : 1,
              }}
            >
              <span
                style={{
              ...
              >
                {MONTHS_FULL_DASH[m.month].slice(0, 3)}
              </span>
            </div>
          );
        })}
      ...
      {open && (
        <div
          style={{
            marginTop: 22,
            paddingTop: 20,
            borderTop: `1px solid ${D.color.line}`,
            display: "flex",
        ...
        >
          {goal.mode === "milestone" && (
            ...
            >
              Goal: save{" "}
              <span style={{ color: D.color.text }}>
                {HZ.eurUnit(goal.targetTotal, { cents: false })}
              </span>{" "}
              by {MONTHS_FULL_DASH[goal.targetDate.month]}{" "}
              {goal.targetDate.year} · auto-split across tracked accounts by
              balance
            </div>
          )}
          {HZ.trackableAccountIds.map((id, i) => {
            const a = HZ.accountById(id);
            const p = goal.perAccount.find((x) => x.id === id);
            const pct =
              p.tracked && p.cumulativeTarget > 0
                ? Math.min(100, (p.cumulativeActual / p.cumulativeTarget) * 100)
                : 0;
            return (
              <div
                key={id}
                style={{
                  ...
                  gap: 12,
                  padding: "12px 10px",
                  opacity: p.tracked ? 1 : 0.5,
                  borderBottom:
                    i === HZ.trackableAccountIds.length - 1
                      ? "none"
              ...
              >
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  >
                    ...
                    {!p.tracked && <Badge tone="neutral">Not tracked</Badge>}
                  </div>
                  {p.tracked ? (
                    <>
                      ...
                      >
                        <Money
                          cents={p.cumulativeActual}
                          size="sm"
                          color={
                            p.cumulativeActual >= p.cumulativeTarget
                              ? D.color.pos
                              : D.color.text
                          }
                        />{" "}
                ...
                <div style={{ textAlign: "right" }}>
                  {p.tracked && (
                    ...
                    >
                      {HZ.eurUnit(p.target, { cents: false })}
                      <span
                        style={{
                          color: D.color.textDim,
                          fontSize: 11,
                          fontFamily: D.font.ui,
                        }}
                      >
                        {" "}
                        /mo
...
function TrajectoryHistoryLink({ go }) {
  const [h, setH] = React.useState(false);
  return (
    <button
      onClick={() => go("history", null, null)}
      ...
      style={{
        ...
        alignItems: "center",
        gap: 6,
        color: h ? D.color.text : D.color.textDim,
        ...D.type.body,
        fontSize: 12.5,
        ...
        actions={
          <>
            <Button
              ...
              icon="download"
              onClick={() =>
                ui.notify("Backup saved · horizon-2026-07-14.db", {
                  variant: "success",
                })
              }
            >
      ...
      <Card pad={24} style={{ marginTop: 22 }}>
        ...
        >
          ...
          >
            ...
            <span style={{ width: 1, height: 14, background: D.color.line }} />
            <TrajectoryHistoryLink go={go} />
          </div>
        </div>
        <TrajectoryChart

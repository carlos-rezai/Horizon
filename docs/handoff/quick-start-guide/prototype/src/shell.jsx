...
function SidebarClock() {
  const [now, setNow] = React.useState(new Date(2026, 6, 14, 9, 41, 0));
  React.useEffect(() => {
  ...
  return (
    <div
      style={{
        marginTop: 8,
        paddingTop: 8,
        borderTop: `1px solid ${S.color.line}`,
      }}
    >
      <button
        onClick={onClick}
        ...
        style={{
          ...
          padding: "8px 12px",
          borderRadius: S.radius.md,
          color: h ? S.color.textMuted : S.color.textDim,
          background: h ? S.color.ink2 : "transparent",
          fontFamily: S.font.ui,
          fontSize: 13,
          fontWeight: 500,
          transition: "all .14s ease",
      ...
      >
        <span style={{ display: "grid", placeItems: "center", width: 18 }}>
          <S_Icon name="book" size={15} stroke={2} color={S.color.textDim} />
        </span>
        Help &amp; manual
      </button>
    </div>
...
function Sidebar({ route, go, onOpenManual }) {
  const nav = [
  ...
  return (
    ...
    >
      ...
      <NavItem
        ...
        onClick={() => go("settings")}
      />
      <ManualLink onClick={onOpenManual} />
    </aside>
  );
}

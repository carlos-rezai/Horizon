...
/* ============================================================
   Edit Savings Goal
   Milestone mode (target amount + date, auto-split by account balance)
   or Manual mode (set monthly target per trackable account directly).
   Editing a value in Milestone mode converts to Manual.
   ============================================================ */
function SavingsGoalModal({ onClose, config, onSave }) {
  const HZ = window.HZ;
  const ids = HZ.trackableAccountIds;
  const [mode, setMode] = React.useState(config.mode);
  const [targetTotal, setTargetTotal] = React.useState(
    (config.targetTotal / 100).toFixed(2)
  );
  const [targetMonth, setTargetMonth] = React.useState(
    `${config.targetDate.year}-${String(config.targetDate.month + 1).padStart(2, "0")}`
  );
  const [manualMonthly, setManualMonthly] = React.useState(() => {
    const base = {};
    ids.forEach((id) => {
      base[id] = (
        ((config.manualMonthly && config.manualMonthly[id]) || 0) / 100
      ).toFixed(2);
    });
    return base;
  });
  ...
  const targetTotalCents =
    Math.round(parseFloat(targetTotal.replace(",", ".")) * 100) || 0;
  const [ty, tm] = targetMonth.split("-").map(Number);
  const targetDate = { year: ty, month: (tm || 1) - 1 };

  const derivedPreview = React.useMemo(() => {
    if (!targetTotalCents || !ty) return {};
    return HZ.computeSavingsGoal({
      mode: "milestone",
      targetTotal: targetTotalCents,
      targetDate,
      startedAt: config.startedAt,
      manualMonthly: {},
    }).monthly;
  }, [targetTotalCents, targetMonth]);

  const switchToManual = () => {
    setManualMonthly(() => {
      const base = {};
      ids.forEach((id) => {
        base[id] = ((derivedPreview[id] || 0) / 100).toFixed(2);
      });
      return base;
    });
    setMode("manual");
  ...
  const setMonthlyValue = (id, v) => {
    if (mode === "milestone") switchToManual();
    setManualMonthly((m) => ({ ...m, [id]: v }));
  };

  const displayValue = (id) =>
    mode === "milestone"
      ? ((derivedPreview[id] || 0) / 100).toFixed(2)
      : manualMonthly[id];
  const anyManualPositive = ids.some(
    (id) =>
      (Math.round(
        parseFloat((manualMonthly[id] || "0").replace(",", ".")) * 100
      ) || 0) > 0
  );
  const canSubmit =
    mode === "milestone" ? targetTotalCents > 0 && !!ty : anyManualPositive;
    ...
    const cfg =
      mode === "milestone"
        ...
        : {
            ...
            startedAt: config.startedAt,
            manualMonthly: Object.fromEntries(
              ids.map((id) => [
                id,
                Math.round(
                  parseFloat((manualMonthly[id] || "0").replace(",", ".")) * 100
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        ...
        {mode === "milestone" ? (
          <>
            ...
            >
              Set a total amount and a target date — Horizon splits the monthly
        ...
        <Field
          label={
            mode === "milestone"
              ? "Auto-split by account"
              : "Monthly target per account"
          }
          hint={
            mode === "milestone"
              ? "edit a value to switch to Manual"
              : undefined
          }
        >
            ...
            {ids.map((id) => {
              ...
              return (
                <div
                  ...
                  style={{
                    display: "grid",
                    gridTemplateColumns: "18px 1fr 140px",
                    alignItems: "center",
                    gap: 10,
                  }}
                >
                  ...
                  <MD_Input
                    ...
                    onChange={(e) => setMonthlyValue(id, e.target.value)}
                    prefix="€"
                  />
                </div>
              );
...
Object.assign(window, { SavingsGoalModal });

...
## Performance + UX Polish (new)

| Term                             | Definition                                                                                                                                                                                                                 | Aliases to avoid                       |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------- |
| **First-Interaction Stutter**    | The app-wide hitch where a view hitches the _first_ time it renders after navigation (e.g. clicking through months), then runs smooth — the headline problem this pass diagnoses & fixes                                   | Lag, jank, first-load stutter          |
| **Cache-Context**                | The lightweight, hand-rolled in-memory cache provider that slow-changing global resources (accounts, projection, categories, recurring) read through instead of each owning a `useEffect` fetch — no new dependency        | Query cache, store, data layer         |
| **Stale-While-Revalidate (SWR)** | The **Cache-Context** freshness rule: reads paint instantly from cache while a background refetch always runs and reconciles — self-healing, so a forgotten dependency is a one-frame flicker, never a persistent lie      | Cache invalidation, background refresh |
| **Explicit Bump**                | A targeted cache invalidation fired by a high-impact mutation (transaction CRUD → `accounts` + `projection`) on top of **SWR**, so those specific resources feel instant rather than one-frame-stale                       | Manual invalidation, force refresh     |
| **Primary Trio**                 | The three daily-driver, heaviest views — **Dashboard**, **Plan (Outlook)**, **Month Overview** — that receive the full polish treatment (profiling, bespoke skeletons, transitions, optimistic flows)                      | Main views, hot views                  |
| **Secondary View**               | Account Detail, History, Import, and Settings — views that receive only _inherited_ wins from shared infrastructure (**Cache-Context**, the **Skeleton** primitive, incidental memoization), never bespoke per-view polish | Minor view, cold view                  |
| **Skeleton**                     | The new `src/primitives/Skeleton/` primitive rendering a layout-matched placeholder while data loads — mirrors a view's real shape so content fills in without reflow                                                      | Loader, shimmer, placeholder           |
| **Progressive Reveal**           | The loading approach that replaces the all-or-nothing gate (one `<Spinner />` until every query resolves) so each section lights up the instant its own data is ready                                                      | Partial load, incremental render       |
| **Optimistic Update**            | The Month-page mutation feel where a transaction create/edit/delete applies to local state immediately, rolls back + notifies on failure, and swaps a provisional id on the create response                                | Instant update, optimistic UI          |
| **Cold-First-Click Trace**       | The profiling protocol that records a performance trace _while clicking a view cold the first time_ — the only capture that surfaces **First-Interaction Stutter**, which warm traces miss                                 | Cold trace, first-run profile          |

## Relationships (Performance + UX Polish additions)

- The **Cache-Context** serves every **Secondary View** the same slow-changing resources it serves the **Primary Trio** — the tiering is about _bespoke_ polish, not about who gets the cache
- **SWR** is the baseline for all cached resources; an **Explicit Bump** is layered on top only for the frequent, high-impact transaction mutations

...
async function createAccount(kind = 'Girokonto', name = 'Main', openingBalance = 500000) {
...
async function postTransfer(fromAccountId: string, toAccountId: string, amount: number) {
...
describe('POST /transfers', () => {
  it('creates two transaction legs sharing the same transferId', async () => {
    ...
    const sourceTxs = await request(app).get(`/accounts/${source._id}/transactions`)
    const destTxs = await request(app).get(`/accounts/${dest._id}/transactions`)
  ...
  it('returns 404 when the destination account does not exist', async () => {
    ...
    const res = await postTransfer(source._id, '000000000000000000000000', 70000)
...
describe('DELETE /transfers/:transferId', () => {
  it('removes both transaction legs', async () => {
    ...
    const res = await request(app).delete(`/transfers/${transfer.body.transferId}`)
